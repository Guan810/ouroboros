...
"""
Ouroboros 本地启动器
替代 colab_launcher.py，无需 Google Colab 和 Google Drive。
状态存储在本地 ~/.ouroboros/ 目录。
...
def ensure_claude_code_cli() -> bool:
    ...
    if has_cli:
        return True
    subprocess.run(
        ["bash", "-lc", "command -v npm >/dev/null 2>&1 && npm install -g @anthropic-ai/claude-code"],
        check=False,
    )
...
def get_required(name: str) -> str:
    ...
    assert v, f"Missing required config: {name}  (请在 .env 文件中填写)"
    return v
...
GITHUB_USER        = get_required("GITHUB_USER")
...
ANTHROPIC_BASE_URL = get_optional("ANTHROPIC_BASE_URL")
LLM_BASE_URL       = get_optional("LLM_BASE_URL")

import re
_raw_budget = os.environ.get("TOTAL_BUDGET", "10")
_clean_budget = re.sub(r'[^0-9.\-]', '', _raw_budget)
TOTAL_BUDGET_LIMIT = float(_clean_budget) if _clean_budget else 10.0

MAX_WORKERS       = int(get_optional("OUROBOROS_MAX_WORKERS", "3"))
MODEL_MAIN        = get_optional("OUROBOROS_MODEL", "gpt-4o")
MODEL_CODE        = get_optional("OUROBOROS_MODEL_CODE", "gpt-4o")
MODEL_LIGHT       = get_optional("OUROBOROS_MODEL_LIGHT", DEFAULT_LIGHT_MODEL or "gpt-4o-mini")
SOFT_TIMEOUT_SEC  = max(60,  int(get_optional("OUROBOROS_SOFT_TIMEOUT_SEC",  "600")))
HARD_TIMEOUT_SEC  = max(120, int(get_optional("OUROBOROS_HARD_TIMEOUT_SEC", "1800")))
DIAG_HEARTBEAT_SEC = max(0,  int(get_optional("OUROBOROS_DIAG_HEARTBEAT_SEC", "30")))
DIAG_SLOW_CYCLE_SEC = max(0, int(get_optional("OUROBOROS_DIAG_SLOW_CYCLE_SEC", "20")))
BUDGET_REPORT_EVERY_MESSAGES = 10

# 写回 env 供子进程使用
os.environ["LLM_API_KEY"]              = LLM_API_KEY
os.environ["OPENAI_API_KEY"]           = OPENAI_API_KEY or LLM_API_KEY
...
def _handle_supervisor_command(text: str, chat_id: int, tg_offset: int = 0):
    ...
    if lowered.startswith("/restart"):
        st2 = load_state()
        st2["session_id"] = uuid.uuid4().hex; st2["tg_offset"] = tg_offset
        save_state(st2)
...
# ----------------------------
# 8) 主循环（与 colab_launcher.py 相同）
# ----------------------------
log.info("🐍 Ouroboros local launcher started. State dir: %s", DRIVE_ROOT)
log.info("   Telegram bot ready. Send a message to register as owner.")
    ...
    for upd in updates:
        ...
        if agent._busy:
            ...
            if agent._busy:
                ...
                _t = threading.Thread(target=_run_task_and_resume, args=(chat_id, final_text, _batched_image), daemon=True)
                try:
                    _t.start()
